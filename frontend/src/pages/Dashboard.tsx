...
export default function Dashboard() {
  ...
  return (
    <div className="page-enter">
      <PageHeader title="Dashboard" description="Welcome to SEVAK Admin Dashboard" />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-3">
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="h-16 w-full rounded-xl bg-muted/20 animate-pulse" />
                ))
              ) : (
                recentActivities.map((activity) => (
                  ...
                  >
                    <div>
                      <p className="font-medium">{activity.month} {activity.year}</p>
                    ...
                    <StatusBadge variant={activity.isActive ? "active" : "inactive"}>
        ...
        <GlassCard variant="elevated" className="animate-slide-up" style={{ animationDelay: "350ms" }}>
          <GlassCardHeader>
            <GlassCardTitle className="flex items-center gap-2">
          ...
          <GlassCardContent>
            <div className="space-y-3">
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-12 w-full rounded-md bg-muted/20 animate-pulse" />)
              ) : (
                users.slice(0, 5).map((user) => {
                  const role = roles.find((r) => r.id === user.roleId);
                  return (
                    <div key={user.id} className="flex items-center justify-between rounded-xl bg-muted/30 p-3 transition-colors hover:bg-muted/50">
